...
class Controller:
    def __init__(self, robot):        
        ...
        for i in range(8):
            ...
            self.proximity_sensors.append(self.robot.getDevice(sensor_name)) #higher number, the closer the object is. I.E. 60 = far away, 250 is very close 
    ...
    def calculate_fitness(self):
        ...
        for i in range(8):
            if self.proximity_sensors[i].getValue()>400: #400 to tune
               ...
               print("forward fitness",self.forward_fitness)

        ### Define the fitness function to avoid spining behaviour
        #spinningFitness = ?
        if self.velocity_right*self.velocity_left <=0:
           self.spin_fitness -= self.velocity_right+self.velocity_left
             
        ### Define the fitness function of this iteration which should be a combination of the previous functions         
        combinedFitness = self.spin_fitness+self.collision_fitness+(5*self.forward_fitness)
    ...
    def run_robot(self):        
        ...
        while self.robot.step(self.time_step) != -1:
            ...
            for i in range(8):
                ### Select the distance sensors that you will us        
